...
TIPS = {
    ...
    ('civ', 'nor'): '哈宝绝杀，挪威淘汰赛首胜',
    ('fra', 'swe'): '姆巴佩双响，法国完胜瑞典',
}
...
def update_data_js(new_data):
    ...
    for match_data in new_data:
        ...
        home = parsed['home']
        away = parsed['away']
        score = f"{parsed['home_score']}-{parsed['away_score']}"
        ...
        tip = TIPS.get((home_id, away_id), parsed['highlight'])

        # 替换 score, status, winner
        pattern = rf"(home: '{home_id}', away: '{away_id}',\s+score: ')[^']+'(\s+status: ')[^']+'(\s+winner: ')[^']+'"
        def replacer(m):
            nonlocal updated
            updated = True
            return f"{m.group(1)}{score}{m.group(2)}'finished'{m.group(3)}'{winner}'"
...
if __name__ == '__main__':
    ...
    for m in data:
        ...
        icon = "✅" if parsed['status'] == '完场' else "⏳"
        print(f"  {icon} {parsed['home']} {parsed['home_score']}-{parsed['away_score']} {parsed['away']} | {parsed['status']}")
